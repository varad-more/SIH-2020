...
##Articles
sql = "CREATE TABLE Articles (Article_Id int NOT NULL,Article_url VARCHAR(30),Article_title VARCHAR(30),   Article_keywords VARCHAR(30),Article_content VARCHAR(30),Article_author VARCHAR(30),Article_old_rank int NOT NULL,Article_new_rank int NOT NULL,Article_error VARCHAR(5),Article_verified BOOLEAN NULL)"
cursor.execute(sql)
...
##Pages
sql = "CREATE TABLE Pages (Page_Id int NOT NULL,Page_url VARCHAR(30),Page_keywords VARCHAR(30),Page_old_rank int NOT NULL,Page_new_rank int NOT NULL)"
cursor.execute(sql)

cursor.execute("INSERT INTO Pages (Page_Id ,Page_url, Page_keywords, Page_old_rank, Page_new_rank) VALUES ('1','url','keywords','2','4')")

##Websites
sql = "CREATE TABLE Websites (Web_Id int NOT NULL,Web_url VARCHAR(30),Web_old_rank int NOT NULL,Web_new_rank int NOT NULL)"
cursor.execute(sql)

cursor.execute("INSERT INTO Websites (Web_Id, Web_url, Web_old_rank, Web_new_rank) VALUES ('1','url','2','4')")

##Company
sql = "CREATE TABLE Company (Company_name TEXT NULL,Securities_ex VARCHAR(10),Company_web_link VARCHAR(30),Op_timeline VARCHAR(10),Trading_location VARCHAR(10))"
cursor.execute(sql)
...
##Securities
sql = "CREATE TABLE Securities (Company_name TEXT NULL,Security_type VARCHAR(10),ISIN VARCHAR(10),Trade_volume int,Listed_on_exchange VARCHAR(20),Exchange_symbol VARCHAR(10))"
cursor.execute(sql)
...
##Table_1
sql = "CREATE TABLE Table_1 (Company_name TEXT NULL,Parent_link VARCHAR(20),file_url VARCHAR(20),publish_date VARCHAR(5))"
cursor.execute(sql)

cursor.execute("INSERT INTO Table_1 (Parent_link,file_url,publish_date) VALUES ('e','f','1')")

##Dashboard
sql = "CREATE TABLE Dashboard (Date_ca TEXT NULL,Company_name TEXT NULL,CA_name TEXT NULL,Security_id_type TEXT NULL,Id_value TEXT NULL,Ex_date TEXT NULL,Rec_date TEXT NULL,Pay_date TEXT NULL, Other VARCHAR(30),Exception BOOLEAN NULL,Remarks VARCHAR(10))"
cursor.execute(sql)
